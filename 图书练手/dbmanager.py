...
if 'db' not in os.listdir(home):
    os.mkdir(os.path.join(home, 'db')) #生成db文件夹
...
class userDbManager(DbManager):
    ...
    def initDatabase(self):
        ...
        print(md5password)
        self.addAdminUser('admin01','mg',md5password)
        ...
        md5password = m2.hexdigest()
        print(md5password)
        self.addUser('user01','user',md5password)
...
class BookDbManager(DbManager):
    ...
    def borrowOrReturnBook(self,BookId,borrowflag = 1):
        if borrowflag == 1:
            self.cursor.execute("UPDATE Book SET NumStorage=NumStorage-1,NumCanBorrow=NumCanBorrow-1 WHERE "
