...
class AddOrDropManager(DbManager):
    ...
    def insertValue(self, BookID, time, AddorDrop, addBookNum):
        insertData = self.cursor.execute(
            "INSERT INTO AddOrDrop VALUES ('%s','%s',%d,%d)" % (BookID, time, AddorDrop, addBookNum))
        self.db.commit()
...
class BookDbManager(DbManager):
    ...
    def addBOOK(self, BookName, BookID, Auth, Category, Publisher, PublishTime, NumStorage, NumCanBorrow, NumBorrowed):
        ...
        insertData = self.cursor.execute("""INSERT INTO Book 
                ...
                ('{0}', '{1}', '{2}','{3}','{4}','{5}','{6}','{7}','{8}')
                """.format(BookName, BookID, Auth, Category, Publisher, PublishTime, NumStorage, NumCanBorrow,
                           NumBorrowed))
        self.db.commit()
    ...
    def updateBookinfo(self, addBookNum, bookId, addFlag=1):
        if addFlag == 1:
            self.cursor.execute(
                "UPDATE Book SET NumStorage=NumStorage+%d,NumCanBorrow=NumCanBorrow+%d WHERE BookID='%s'" % (
    ...
    def borrowOrReturnBook(self, BookID, borrowflag=1):
        if borrowflag == 1:
            fetchedData = self.cursor.execute(
                "UPDATE Book SET NumCanBorrow=NumCanBorrow-1,NumBorrowed=NumBorrowed+1 WHERE BookID='%s'" % BookID)
        else:
            fetchedData = self.cursor.execute(
                "UPDATE Book SET NumCanBorrow=NumCanBorrow+1,NumBorrowed=NumBorrowed-1 WHERE BookID='%s'" % BookID)
        self.db.commit()
...
class UserDbManager(DbManager):
    ...
    def borrowOrReturnBook(self, userid, borrow=1):
        if borrow == 1:
            fetchedData = self.cursor.execute(
                "UPDATE User SET TimesBorrowed=TimesBorrowed+1,NumBorrowed=NumBorrowed+1 WHERE userid='%s'" % userid)
        else:
            fetchedData = self.cursor.execute(
                "UPDATE User SET TimesBorrowed=TimesBorrowed-1,NumBorrowed=NumBorrowed-1 WHERE userid='%s'" % userid)
        self.db.commit()
...
def testuserdb():
    ...
    userDb.addAdminUser('1', 'admin', '123456')
    userDb.addAdminUser('2', 'admin1', '123456')
